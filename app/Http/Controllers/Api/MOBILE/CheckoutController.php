...
class CheckoutController extends Controller
{
        ...
        $mass_communication = [
            [
                ...
                "points" => [
                    ...
                    // [30.113771288317199, 31.606623695118301],
                    [30.114435622750701, 31.609196124055401],
                    [30.113969660590001, 31.606520424934999],
                    [30.111352527184199, 31.6070705076141],
                    [30.111822470450601, 31.609813116246599],
                    [30.114435622750701, 31.609196124055401],
                ]
            ]
        ];
    ...
    {
        // Check if the user checked out before
        $loggedInUserId = auth()->id();
        $attendance = Attendance::where('employee_id', $loggedInUserId)
            ->where('last_att_status', 'lst-1')
            ->orderBy('id', 'desc')
            ->first();
        if (!$attendance) {
            return response()->json(['status' => 403, 'message' => 'Cannot check out, checkin first!'], 200);
        }
            ...
            foreach ($polygons as $polygon) {
                foreach ($polygon as $zone) {
                    ...
                    if ($this->isInsidePolygon($zone["points"], $userLocation)) {
                        ...
                        $attendance->last_att_status = "lst-2";
                        $attendance->save();

                        return response()->json(['status' => 200, 'message' => 'Checkout saved successfully!'], 200);
                    }
                }
            }
